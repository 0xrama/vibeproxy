...
class ThinkingProxy {
    ...
    private let stateQueue = DispatchQueue(label: "local.vibeproxy.thinking-proxy-state")

    var vercelConfig = VercelGatewayConfig(enabled: false, apiKey: "")

    /// Fired for every request forwarded to the backend (used for idle tracking).
    var onRequestActivity: (() -> Void)?

    /// When set, invoked synchronously before forwarding to the backend so the owner
    /// can cold-start the CLIProxyAPI process on demand (eco mode).
    /// Returns true when the backend is (or has become) reachable.
    var onBackendNeeded: (() -> Bool)?
    ...
    private func forwardRequest(method: String, path: String, version: String, headers: [(String, String)], body: String, thinkingEnabled: Bool = false, originalConnection: NWConnection, retryWithApiPrefix: Bool = false) {
        ...
        if let onBackendNeeded, !onBackendNeeded() {
            NSLog("[ThinkingProxy] Backend unavailable after cold-start attempt")
            sendError(to: originalConnection, statusCode: 503, message: "VibeProxy backend is not running")
            return
        }
