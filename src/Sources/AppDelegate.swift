...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, UNUserNotificationCenterDelegate {
    ...
    // MARK: - Eco mode (on-demand backend)

    private let efficiency = EfficiencySettings.shared
    private var lastBackendActivity = Date()
    private var idleChecker: DispatchSourceTimer?
    private var ecoWakeSuspended = false
    private let coldStartStateQueue = DispatchQueue(label: "local.vibeproxy.cold-start")
    private var coldStartTriggered = false

    private enum EcoTiming {
        static let idleCheckInterval: TimeInterval = 30
        static let coldStartTimeout: TimeInterval = 20
        static let coldStartPollInterval: TimeInterval = 0.05
    }

    override init() {
        super.init()
        EfficiencySettings.shared.onChange = { [weak self] in
            DispatchQueue.main.async {
                self?.applyEfficiencyModeTransition()
            }
        }
    }
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Wire eco-mode hooks: idle tracking and on-demand backend start
        thinkingProxy.onRequestActivity = { [weak self] in
            self?.noteBackendActivity()
        }
        thinkingProxy.onBackendNeeded = { [weak self] in
            self?.ensureBackendReachable() ?? false
        }
        startIdleChecker()
    ...
    @objc func toggleServer() {
        if serverManager.isRunning {
            ...
            startServer()
            // Classic mode defers to pollForProxyReadiness; eco mode warms the backend immediately
            // so the first request doesn't pay the cold start.
            if efficiency.onDemandBackendEnabled {
                // Manual warm start: give it a full idle window before sleeping.
                noteBackendActivity()
                serverManager.start { [weak self] _ in
                    DispatchQueue.main.async {
    ...
    func startServer() {
        ecoWakeSuspended = false
        // Start the thinking proxy first (port 8317)
        thinkingProxy.start()

        // In eco mode the backend starts lazily on the first request.
        if efficiency.onDemandBackendEnabled {
    ...
    private func noteBackendActivity() {
        coldStartStateQueue.async { [weak self] in
            self?.lastBackendActivity = Date()
        }
    }

    /// Ensures the CLIProxyAPI backend is reachable, cold-starting it when eco mode allows.
    /// Called synchronously from ThinkingProxy's forwarding queue; blocks up to coldStartTimeout.
    private func ensureBackendReachable() -> Bool {
        noteBackendActivity()
        if TCPLocalPortProbe.isPortOpen(port: 8318) {
            return true
        }

        let shouldTriggerStart = coldStartStateQueue.sync { () -> Bool in
            guard !ecoWakeSuspended, efficiency.onDemandBackendEnabled else {
                return false
            }
        ...
        if shouldTriggerStart {
            NSLog("[EcoMode] Cold-starting backend on demand")
            DispatchQueue.main.async { [weak self] in
                self?.serverManager.start { [weak self] success in
                    DispatchQueue.main.async {
                        if success {
                            ...
                            NSLog("[EcoMode] Backend cold start failed")
                            self?.coldStartStateQueue.async { self?.coldStartTriggered = false }
                        }
                    }
                }
            }
        ...
        let deadline = Date().addingTimeInterval(EcoTiming.coldStartTimeout)
        while Date() < deadline {
            if TCPLocalPortProbe.isPortOpen(port: 8318) {
                coldStartStateQueue.async { [weak self] in
                    self?.coldStartTriggered = false
                }
                return true
            }
            Thread.sleep(forTimeInterval: EcoTiming.coldStartPollInterval)
        }

        NSLog("[EcoMode] Backend did not become reachable within %.0fs", EcoTiming.coldStartTimeout)
        coldStartStateQueue.async { [weak self] in
            self?.coldStartTriggered = false
        }
        return false
    ...
    private func startIdleChecker() {
        idleChecker?.cancel()
        let checker = DispatchSource.makeTimerSource(queue: DispatchQueue.main)
        checker.schedule(deadline: .now() + EcoTiming.idleCheckInterval, repeating: EcoTiming.idleCheckInterval)
    ...
    private func checkIdleTimeout() {
        guard efficiency.onDemandBackendEnabled,
              !ecoWakeSuspended,
              serverManager.isRunning,
              !isColdStartInProgress else {
            return
        }

        let idleInterval = coldStartStateQueue.sync { Date().timeIntervalSince(lastBackendActivity) }
        guard idleInterval >= efficiency.idleTimeoutSeconds else {
            return
        }

        let minutes = Int(efficiency.idleTimeoutSeconds / 60)
        NSLog("[EcoMode] Backend idle for %.0fs (limit %dm) — stopping backend, proxy stays listening", idleInterval, minutes)
        serverManager.stop()
        showNotification(title: "Backend Sleeping", body: "No traffic for \(minutes) min. It wakes instantly on the next request.")
    }

    private var isColdStartInProgress: Bool {
        coldStartStateQueue.sync { coldStartTriggered }
    }

    /// Reacts to efficiency preference changes so runtime state matches the selected mode.
    private func applyEfficiencyModeTransition() {
        guard efficiency.onDemandBackendEnabled else {
            // Classic mode: if the proxy is listening but the backend is down, warm it up now.
            if thinkingProxy.isRunning && !serverManager.isRunning && !ecoWakeSuspended {
                pollForProxyReadiness(attempts: 0, maxAttempts: 60, intervalMs: 50)
            }
            return
        }
        // Entering eco mode: nothing to do; the backend will idle-stop after the timeout.
    }
    ...
    func stopServer() {
        // In eco mode a manual stop suspends wake-on-request until the next manual start;
        // the local proxy stays listening so the app remains responsive.
        if efficiency.onDemandBackendEnabled {
            ecoWakeSuspended = true
            serverManager.stop()
        } else {
    ...
    @objc func updateMenuBarStatus() {
        let ecoEnabled = efficiency.onDemandBackendEnabled

        // Update status items
        if let serverStatus = menu.item(at: 0) {
            if serverManager.isRunning {
                serverStatus.title = "Server: Running (port \(thinkingProxy.proxyPort))"
            } else if ecoEnabled && thinkingProxy.isRunning {
                serverStatus.title = "Server: On-demand (idle, port \(thinkingProxy.proxyPort))"
            } else {
                serverStatus.title = "Server: Stopped"
            }
        }
        ...
        if let button = statusItem.button {
            let isServing = serverManager.isRunning
            let iconName = isServing ? "icon-active.png" : "icon-inactive.png"
            let fallbackSymbol = isServing ? "network" : "network.slash"
            
            if let icon = IconCatalog.shared.image(named: iconName, resizedTo: NSSize(width: 18, height: 18), template: true) {
                button.image = icon
            } else {
                let fallback = NSImage(systemSymbolName: fallbackSymbol, accessibilityDescription: isServing ? "Running" : "Stopped")
                fallback?.isTemplate = true
                button.image = fallback
            }
            button.toolTip = serverManager.isRunning
                ? "VibeProxy — serving"
                : (ecoEnabled && thinkingProxy.isRunning ? "VibeProxy — on-demand (wakes on first request)" : "VibeProxy")
        }
    }
    ...
    func showNotification(title: String, body: String) {
        ...
        let request = UNNotificationRequest(
            identifier: "local.vibeproxy.\(UUID().uuidString)",
            content: content,
            trigger: nil
        )
