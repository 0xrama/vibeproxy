...
class ServerManager: ObservableObject {
    ...
    private let processQueue = DispatchQueue(label: "local.vibeproxy.server-process", qos: .userInitiated)
    private let credentialMutationQueue = DispatchQueue(label: "local.vibeproxy.credential-mutations", qos: .userInitiated)
    private let configInputStateQueue = DispatchQueue(label: "local.vibeproxy.config-input-state", qos: .userInitiated)
    private let configResolutionQueue = DispatchQueue(label: "local.vibeproxy.config-resolution", qos: .userInitiated)
    ...
    // MARK: - User-defined OpenAI-compatible providers

    /// Adds (or replaces) an OpenAI-compatible provider in the user config and optionally stores its API key.
    func addCustomProvider(
        ...
        credentialMutationQueue.async { [weak self] in
            ...
            // Validate inputs
            guard let providerID = ConfigComposer.slugifiedProviderID(fromTitle: title) else {
                DispatchQueue.main.async {
                    completion(false, "Enter a provider name containing letters or numbers.")
                }
                return
            }
            if ProviderCatalog.reservedCustomProviderKeys.contains(providerID) {
                DispatchQueue.main.async {
                    completion(false, "'\(providerID)' is reserved and cannot be used as a provider name.")
                }
                return
            }
            guard let normalizedBaseURL = ConfigComposer.normalizedString(baseURL),
                  let url = URL(string: normalizedBaseURL),
                  let scheme = url.scheme?.lowercased(),
                  scheme == "http" || scheme == "https",
                  url.host != nil else {
                DispatchQueue.main.async {
                    completion(false, "Enter a valid base URL such as https://api.example.com/v1")
                }
                return
            }
            let parsedModels = ConfigComposer.parseModelSpecs(modelSpecs)
            guard parsedModels.error == nil else {
                DispatchQueue.main.async {
                    completion(false, parsedModels.error ?? "Invalid model list")
                }
                return
            }

            let existingProviderIDs = Set(ConfigComposer.parseCustomProviders(
                from: (try? self.loadUserConfigRootForWriting()) ?? [:],
                reservedProviderIDs: []
            ).map { $0.id })
            .union(self.currentCustomProviderIDs())
            if existingProviderIDs.contains(providerID) {
                DispatchQueue.main.async {
                    completion(false, "A provider named '\(providerID)' already exists.")
                }
                return
            }

            do {
                var userRoot = try self.loadUserConfigRootForWriting()
                ...
                try self.writeUserConfigRoot(userRoot)
                self.addLog("✓ Added custom provider: \(providerID)")

                if !apiKey.isEmpty {
                    let saveResult = try self.customProviderCredentialStore.save(providerID: providerID, apiKey: apiKey)
                    let recordLabel: String
                    switch saveResult {
                    case .created(let record), .alreadyPresent(let record), .reenabled(let record):
                        recordLabel = record.label
                    }
                    self.addLog("✓ Saved API key for custom provider: \(recordLabel)")
                }

                self.refreshAuthBackedConfiguration()
                DispatchQueue.main.async {
                    completion(true, apiKey.isEmpty
                        ? "✓ Provider '\(title)' added. Add an API key to start using it."
                        : "✓ Provider '\(title)' added with API key.")
                }
            } catch {
                DispatchQueue.main.async {
                    completion(false, "Failed to save provider: \(error.localizedDescription)")
                }
            }
        }
    }

    /// Removes a user-defined OpenAI-compatible provider from the user config (bundled providers cannot be removed here).
    func removeCustomProvider(providerID: String, completion: @escaping (Bool, String) -> Void) {
        credentialMutationQueue.async { [weak self] in
            guard let self else { return }

            do {
                var userRoot = try self.loadUserConfigRootForWriting()
                guard ConfigComposer.removeUserProviderEntry(providerID: providerID, from: &userRoot) else {
                    DispatchQueue.main.async {
                        completion(false, "'\(providerID)' is bundled with the app and cannot be removed. You can disable it with its toggle.")
                    }
                    return
                }
                try self.writeUserConfigRoot(userRoot)
                self.addLog("⚠️ Removed custom provider: \(providerID)")
                self.refreshAuthBackedConfiguration()
                DispatchQueue.main.async {
                    completion(true, "Removed provider '\(providerID)'.")
                }
            } catch {
                DispatchQueue.main.async {
                    completion(false, "Failed to remove provider: \(error.localizedDescription)")
                }
            }
        }
    }

    private func currentCustomProviderIDs() -> Set<String> {
        switch loadBaseConfigRoot() {
        case .success(let baseConfig):
            return Set(ConfigComposer.parseCustomProviders(
                from: baseConfig.root,
                reservedProviderIDs: ProviderCatalog.reservedCustomProviderKeys
            ).map { $0.id })
        case .failure:
    ...
    /// Loads the raw user config (~/.cli-proxy-api/config.yaml) for editing; returns [:] when absent.
    private func loadUserConfigRootForWriting() throws -> [String: Any] {
        ...
        do {
            let content = try String(contentsOfFile: userConfigPath, encoding: .utf8)
            guard let loaded = try Yams.load(yaml: content) else {
                return [:]
            }
            guard let dictionary = ConfigComposer.stringKeyedDictionary(loaded) else {
                throw ConfigResolutionFailure(message: "Config at \(userConfigPath) must be a YAML mapping at the root.")
            }
            return dictionary
        } catch let error as ConfigResolutionFailure {
            throw error
        } catch {
            throw ConfigResolutionFailure(message: "Failed to parse YAML at \(userConfigPath): \(error.localizedDescription)")
        }
    }

    private func writeUserConfigRoot(_ root: [String: Any]) throws {
        let authDir = authDirectoryURL()
        let userConfigPath = authDir.appendingPathComponent(CustomProviderConstants.userConfigFilename)
        do {
            try FileManager.default.createDirectory(at: authDir, withIntermediateDirectories: true)
            let content = try Yams.dump(object: root)
            try content.write(toFile: userConfigPath.path, atomically: true, encoding: String.Encoding.utf8)
            try FileManager.default.setAttributes([.posixPermissions: 0o600], ofItemAtPath: userConfigPath.path)
        } catch {
            throw ConfigResolutionFailure(message: "Failed to write \(userConfigPath.path): \(error.localizedDescription)")
        }
    }
